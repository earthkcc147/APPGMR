...
try:
    ...
    print("[*] ถ้าไม่รู้วิธีติดตั้งคำสั่ง: pip install mechanize; pip install beautifulsoup4")
...
class SPAMMER:
    def __init__(self):
        ...
        # รับค่า email และ password จากผู้ใช้
        self._email = input("[*] อีเมล: ")
    ...
    def login(self):
        ...
        self.br.form["email"] = self._email
        self.br.form["pass"] = self._pasw
        self.br.submit()
    ...
    def _generateTarget(self):
        ...
        # รับชื่อผู้ใช้เป้าหมายจากผู้ใช้
        _targ = input("[*] กรอกชื่อผู้ใช้เป้าหมาย: ")
        _TARGETS = self._url + _targ
        self.br.open(_TARGETS)
        self.r = re.findall('<head><title>(.*?)</title>', self.br.response().read())
        if len(self.r) != 0:
        ...
        # รับจำนวนข้อความที่ต้องการส่ง
        self._counts = int(input("[+] จำนวนที่ต้องการส่ง: "))
        self.br.open(_TARGETS)
        _bs = self.br.response().read()
        _b = bs4.BeautifulSoup(_bs, features="html.parser")
    ...
    def inputMessages(self):
        global _RAHIMIBU
        # รับข้อความที่ต้องการส่ง
        print("[*] ตัวอย่างการใช้: ข้อความจากบอท")
        _msg = input("[+] ข้อความ: ").split(",")
        print("[+] เริ่มต้นการส่งข้อความ!")
        print("[+] กำลังส่ง %s ข้อความไปยัง \033[1;37m\033[31m%s\033[0m" % (self._counts, self.r[0]))
        ...
        for x in range(self._counts):
            ...
            if len(re.findall(r"{}".format(ms), _res)) != 0:
                print(" | \033[1;32m{} -> ส่งแล้ว [{}]\033[0m".format(ms, _RAHIMIBU))
            else:
                print(" | \033[1;37m\033[31m{} -> ส่งไม่สำเร็จ [{}]\033[0m".format(ms, _RAHIMIBU))
