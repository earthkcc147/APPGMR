...
def helper():
        print(bcolors.OKBLUE+"ข้อมูล:"+bcolors.ENDC)
        print("------------")
        print("\t./rapidscan.py example.com: สแกนโดเมน example.com.")
        print("\t./rapidscan.py example.com --skip dmitry --skip theHarvester: ข้ามการทดสอบ 'dmitry' และ 'theHarvester'.")
        print("\t./rapidscan.py example.com --nospinner: ปิดการโหลด/แสดงสปินเนอร์.")
        print("\t./rapidscan.py --update   : อัปเดตเครื่องมือสแกนให้เป็นเวอร์ชันล่าสุด.")
        print("\t./rapidscan.py --help     : แสดงข้อมูลการช่วยเหลือนี้.")
        print(bcolors.OKBLUE+"อินเตอร์แอคทีฟ:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: ข้ามการทดสอบปัจจุบัน.")
        print("\tCtrl+Z: ออกจาก RapidScan.")
        print(bcolors.OKBLUE+"ตำนาน:"+bcolors.ENDC)
        print("--------")
        print("\t["+proc_high+"]: กระบวนการสแกนอาจใช้เวลานาน (ไม่สามารถคาดเดาได้).")
        print("\t["+proc_med+"]: กระบวนการสแกนอาจใช้เวลาน้อยกว่า 10 นาที.")
        print("\t["+proc_low+"]: กระบวนการสแกนอาจใช้เวลาน้อยกว่าหนึ่งหรือสองนาที.")
        print(bcolors.OKBLUE+"ข้อมูลช่องโหว่:"+bcolors.ENDC)
        print("--------------------------")
        print("\t"+vul_info('c')+": ต้องการความสนใจทันทีเนื่องจากอาจทำให้เกิดการละเมิดหรือการหยุดให้บริการ.")
        print("\t"+vul_info('h')+"    : อาจไม่ทำให้เกิดการละเมิดทันที แต่มีโอกาสที่น่ากังวล.")
        print("\t"+vul_info('m')+"  : ผู้โจมตีอาจรวมช่องโหว่หลายๆ ตัวนี้เพื่อโจมตีที่ซับซ้อน.")
        print("\t"+vul_info('l')+"     : ไม่ใช่ปัญหาร้ายแรง แต่ควรได้รับการแก้ไข.")
        print("\t"+vul_info('i')+"    : ไม่ถูกจัดเป็นช่องโหว่, เป็นการแจ้งเตือนข้อมูลที่มีประโยชน์เพื่อพิจารณา.\n")
...
def get_parser():
    ...
    parser.add_argument('-u', '--update', action='store_true', 
                        help='อัปเดต RapidScan')
    parser.add_argument('-s', '--skip', action='append', default=[],
                        help='ข้ามเครื่องมือบางตัว', choices=[t[0] for t in tools_precheck])
    parser.add_argument('-n', '--nospinner', action='store_true', 
                        help='ปิดการโหลดหรือสปินเนอร์')
    parser.add_argument('target', nargs='?', metavar='URL', help='URL ที่ต้องการสแกน', default='', type=str)
    return parser
...
if args_namespace.help or (not args_namespace.update \
    ...
    print("\tจำนวนการตรวจสอบช่องโหว่ทั้งหมด        : "+bcolors.BOLD+bcolors.OKGREEN+str(len(tool_names))+bcolors.ENDC)

    print("\tจำนวนการตรวจสอบช่องโหว่ที่ข้าม        : "+bcolors.BOLD+bcolors.WARNING+str(rs_skipped_checks)+bcolors.ENDC)

    print("\tจำนวนช่องโหว่ที่ตรวจพบทั้งหมด        : "+bcolors.BOLD+bcolors.BADFAIL+str(len(rs_vul_list))+bcolors.ENDC)

    print("\tเวลาทั้งหมดที่ใช้ในการสแกน            : "+bcolors.BOLD+bcolors.OKBLUE+display_time(int(rs_total_elapsed))+bcolors.ENDC)

    print("\n")

    print("\tสำหรับการดีบัก, คุณสามารถดูผลลัพธ์ทั้งหมดที่สร้างโดยเครื่องมือต่างๆ ที่ชื่อว่า "+bcolors.OKBLUE+debuglog+bcolors.ENDC+" ในไดเรกทอรีเดียวกัน.")

    print(bcolors.BG_ENDL_TXT+"[ การสร้างรายงานเสร็จสิ้นแล้ว. ]"+bcolors.ENDC)
