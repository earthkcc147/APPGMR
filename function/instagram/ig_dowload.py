...
INSTAGRAM_USERNAME = input("กรุณาใส่ชื่อผู้ใช้ Instagram: ")

# ------------------------------ไม่ต้องแก้ไขข้างล่างนี้--------------------------------------- #
...
class Instagram_Downloader:
    ...
    def create_download_directory(self):
        try:
            ...
            if not os.path.exists('downloads'):
                os.mkdir('downloads')
                print("สร้างไดเรกทอรี 'downloads' สำเร็จ.")
            
            # สร้างโฟลเดอร์ของผู้ใช้ถ้ายังไม่มี
            if not os.path.exists(f'downloads/{self.username}'):
                os.mkdir(f'downloads/{self.username}')
                print(f"สร้างไดเรกทอรี {self.username} สำเร็จในโฟลเดอร์ 'downloads'.")
        except FileExistsError:
            print(f"โฟลเดอร์ 'downloads' หรือ {self.username} มีอยู่แล้ว.")
    ...
    def write_resume_end_cursor_timeline_media(self):
        f = open("resume_"+self.username+".txt", "w")   # สร้างไฟล์บันทึกตำแหน่งที่ดาวน์โหลดล่าสุด
    ...
    def download_photos(self):
        ...
        for match in self.jsondata.json()[self.apilabel]["user"]["edge_owner_to_timeline_media"]["edges"]: 
            time.sleep(0.5) 
            print("กำลังดาวน์โหลดรูป:\n" +match["node"]["display_url"])
            filename = f'downloads/{self.username}/{self.jsondata.json()[self.apilabel]["user"]["edge_owner_to_timeline_media"]["edges"][i]["node"]["shortcode"]}.jpg'
            try:
                if not os.path.exists(filename ):
                    urllib.request.urlretrieve(match["node"]["display_url"], filename)
                else:
                    print("แจ้งเตือน: "+ filename+ " รูปนี้ดาวน์โหลดแล้ว ข้ามไป.")
            except urllib.error.HTTPError as e:
                print(str(e.code) + " ไม่สามารถดาวน์โหลดรูปได้")
            i = i + 1
    ...
    def download_videos(self):
        ...
        if 'edge_felix_video_timeline' in self.jsondata.json()[self.apilabel]["user"]:
            for match in self.jsondata.json()[self.apilabel]["user"]["edge_felix_video_timeline"]["edges"]: 
                time.sleep(0.5) 
                print("กำลังดาวน์โหลดวิดีโอ:\n" +match["node"]["video_url"])
                filename = f'downloads/{self.username}/{self.jsondata.json()[self.apilabel]["user"]["edge_felix_video_timeline"]["edges"][i]["node"]["shortcode"]}.mp4'
                try:
                    if not os.path.exists(filename ):
                        urllib.request.urlretrieve(match["node"]["video_url"], filename)
                    else:
                        print("แจ้งเตือน: "+ filename+ " วิดีโอนี้ดาวน์โหลดแล้ว ข้ามไป.")
                except urllib.error.HTTPError as e:
                    print(str(e.code) + " ไม่สามารถดาวน์โหลดวิดีโอได้")
                i = i + 1
...
try:
    ...
    while True:
        time.sleep(5) # หยุดพักเพื่อหลีกเลี่ยงการถูกแบน
        user.get_jsondata_instagram()
        user.download_photos()
        user.download_videos()
        ...
        if user.has_next_page() == False:
            ...
            print("เสร็จสิ้น. ดาวน์โหลดรูปภาพ/วิดีโอทั้งหมดจากบัญชี "+INSTAGRAM_USERNAME+" เรียบร้อยแล้ว.")
            break
except:
    print("แจ้งเตือน: สคริปต์หยุดทำงานก่อนกำหนดเนื่องจากจำนวนคำขอที่ส่งไปยัง Instagram API เกินขีดจำกัดประจำวัน.")
    print("กรุณารันสคริปต์ใหม่อีกครั้งในอีกไม่กี่ชั่วโมงข้างหน้าเพื่อดำเนินการต่อจากที่ดาวน์โหลดค้างไว้.")